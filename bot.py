...
class PlayerRefreshView(discord.ui.View):
    def __init__(self, username: str, user_id: int, language: str = 'en'):
        ...
        # Add appropriate button based on language
        if language == 'ru':
            self.refresh_button.label = "Обновить"
        else:
    ...
    async def refresh_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.is_expired():
            if self.language == 'ru':
                error_msg = "⏰ Кнопка обновления истекла (24 часа). Пожалуйста, используйте команду снова."
            else:
                error_msg = "⏰ Refresh button has expired (24 hours). Please use the command again."
        ...
        try:
            # Get the bot instance to access scraper
            bot = interaction.client
            start_time = time.time()
            
            # Scrape fresh player data
            player_data = await bot.scraper.get_player_data(self.username.strip())
            
            if not player_data:
                if self.language == 'ru':
                    embed = discord.Embed(
                        title="❌ Игрок не найден",
                        description=f"Игрок `{self.username}` не найден, попробуйте еще раз.",
                        color=0xff0000
                    )
                else:
                    embed = discord.Embed(
                        title="❌ Player Not Found",
                        description=f"Player `{self.username}` not found, try again.",
                        color=0xff0000
                    )
                
                await interaction.followup.edit_message(interaction.message.id, embed=embed, view=self)
                bot.scraping_failures += 1
                return
            
            # Create updated embed based on language
            if self.language == 'ru':
                embed = await bot._create_player_embed_russian(player_data)
            else:
                embed = await bot._create_player_embed(player_data)
            
            # Update the original message with new data and same view
            new_view = PlayerRefreshView(self.username, self.user_id, self.language)
            await interaction.followup.edit_message(interaction.message.id, embed=embed, view=new_view)
            
            # Update bot statistics
            scraping_time = time.time() - start_time
            bot.total_scraping_time += scraping_time
            bot.scraping_successes += 1
            
        except Exception as e:
            logger.error(f"Error processing refresh: {e}")
...
class RTanksBot(commands.Bot):
    ...
    async def player_command_handler(self, interaction: discord.Interaction, username: str):
        ...
        try:
            ...
            # Create refresh view
            view = PlayerRefreshView(username, interaction.user.id, 'en')
    ...
    async def player_command_handler_russian(self, interaction: discord.Interaction, username: str):
        ...
        try:
            ...
            # Create refresh view with Russian language
            view = PlayerRefreshView(username, interaction.user.id, 'ru')
    ...
    async def _create_player_embed(self, player_data):
        ...
        if player_data['equipment']:
            ...
            if player_data['equipment'].get('protections'):
                protections = ", ".join(player_data['equipment']['protections'])  # Show all protections
